...
from ..utils.job_payload import job_record_to_dict
...
(data) => {
  try {
    ...
    jobs.forEach((j, idx) => {
      ...
      row("Salary", j.salary);
      row("Relevance", (j.relevant_percentage != null ? j.relevant_percentage : 0) + "%");
      row("Skills", Array.isArray(j.skills) ? j.skills.join(", ") : j.skills);
    ...
    foot.style.cssText = "margin-top:10px;font-size:11px;color:#64748b;";
    foot.textContent = "Injected by LangGraph display_jobs_node";
    root.appendChild(foot);

    const mount = document.documentElement || document.body;
    if (!mount) {
      return { ok: false, error: "No document mount node (documentElement/body missing)" };
    }
...
async def _inject_panel(page: Page, jobs: list[JobRecord], errors: list[str]) -> None:
    ...
    payload_jobs = [job_record_to_dict(j) for j in jobs]
    result = await page.evaluate(_PANEL_SCRIPT, {"jobs": payload_jobs, "errors": errors})

    if not isinstance(result, dict):
        raise RuntimeError(f"Panel script returned unexpected value: {result!r}")

    if not result.get("ok"):
        raise RuntimeError(str(result.get("error") or "Panel injection failed in browser"))
