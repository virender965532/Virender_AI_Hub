...
TOP_K = _env_int("DYNAMIC_RAG_TOP_K", 4)
CHUNK_SIZE = _env_int("DYNAMIC_RAG_CHUNK_SIZE", 1000)
CHUNK_OVERLAP = _env_int("DYNAMIC_RAG_CHUNK_OVERLAP", 200)
...
OPENAI_API_KEY = os.getenv("DYNAMIC_RAG_OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY", "")
OPENAI_CLIENT = OpenAI(api_key=OPENAI_API_KEY) if OPENAI_API_KEY else None

_ANSWER_SYSTEM_PROMPT = """You are a helpful assistant answering questions using an uploaded document as the primary source.

Generate a clear, complete answer — do not dump raw context text.

When the question has multiple parts:
1. From the document — Use only the provided context for definitions and document examples. Every claim here MUST include a citation: (Section: <title>, Page: <number>).
...
"""
    ...
    user_prompt = f"""Use the context below from the uploaded document "{document_name}" to answer the question.
...
Write a synthesized answer (not a copy of the context). Include citations for document facts. If the question asks for Python code or custom guardrails, add a practical example section with runnable code inspired by the concepts in the context."""
